...
public class ProxyLogin implements InterfataModulLogin{
	...
	public boolean login(String username, String pass) {
		Integer nrIncercariNereusite = nrIncercari.get(username);//daca nu exista aceasta cheie in map primim null
		if(nrIncercariNereusite == null) { //e prima data cand cineva incearca sa se logheze
			nrIncercari.put(username, 0);
			nrIncercariNereusite = 0;
		}
		if(modulLogin != null) {
			if(nrIncercariNereusite < NR_MAX_INCERCARI) {//deja cand ajungi la 2, faci a 3a incercare
